...
public class AcceptanceTest {
    ...
    public void should_return_vat_calculated_for_bilateral_in_case_uae_lender_and_uae_borrower() {
        FxRateCalculationRepository fxRateCalculationAdapter = Mockito.mock(FxRateCalculationRepository.class);
        ...
        Mockito.when(fxRateCalculationAdapter.getRate(deal.getFeeType(), deal.getDateOfDeal())).thenReturn(fxRate);
        FeeCalculator uaeFeeCalculator = new UAEFeeCalculator(fxRateCalculationAdapter);

        Loan loanFee = uaeFeeCalculator.calculate(deal);
